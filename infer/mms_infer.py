...
def process(args):    
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        cmd = f"""
        PREFIX=INFER HYDRA_FULL_ERROR=1 python infer/new/infer.py -m --config-dir config/ --config-name infer_common decoding.type=viterbi dataset.max_tokens=1440000 distributed_training.distributed_world_size=1 "common_eval.path='{args.model}'" task.data={tmpdir} dataset.gen_subset="{args.lang}:dev" common_eval.post_process={args.format} decoding.results_path={tmpdir} {args.extra_infer_args}
        """
        ...
        while True:
            # 获取子进程输出
            out = p.stdout.readline().decode(sys.stdout.encoding)
